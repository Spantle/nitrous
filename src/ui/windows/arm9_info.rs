...
impl NitrousGUI {
    pub fn show_arm9_info(&mut self, ctx: &egui::Context) {
        egui::Window::new("ARM9 Info")
            ...
            .show(ctx, |ui| {
                ui.collapsing("Register Values", |ui| {
                                    ...
                                    body.row(20.0, |mut row| {
                                        ...
                                        for i in 0..r.len() {
                                            if i < min || i > max {
                                                ...
                                                row.col(|ui| {
                                                    ui.label(format!("{:08X}", registers[i - min]));
                                                });
                                            }
                                        }
                                    });
                ...
                ui.collapsing("Program Status Registers", |ui| {
                    egui_extras::TableBuilder::new(ui)
                        .striped(true)
                        .cell_layout(egui::Layout::left_to_right(egui::Align::Center))
                        .column(egui_extras::Column::exact(60.0))
                        .column(egui_extras::Column::exact(127.0))
