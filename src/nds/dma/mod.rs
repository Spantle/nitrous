...
impl<Bus: BusTrait> DMA<Bus> {
    pub fn read_slice<const T: usize>(&self, addr: usize) -> Option<[u8; T]> {
        match addr {
            ...
            // 0x040000B8 => Some(self.dma0cnt.0.to_bytes::<T>()),
            ...
            // 0x040000C4 => Some(self.dma1cnt.0.to_bytes::<T>()),
            ...
            // 0x040000D0 => Some(self.dma2cnt.0.to_bytes::<T>()),
            ...
            // 0x040000DC => Some(self.dma3cnt.0.to_bytes::<T>()),
    ...
    pub fn check_immediately(&mut self, bus: &mut Bus, shared: &mut Shared) {
        for channel in self.channel.iter_mut() {
            let matches = if Bus::KIND == ArmKind::ARM9 {
