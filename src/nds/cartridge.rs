...
impl Cartridge {
    pub fn load(&mut self, rom: Vec<u8>) -> bool {
        ...
        logger::info("=== Parsing ROM ===");
        logger::info(format!("ROM size: {} bytes", self.rom.len()));

        self.game_title = String::from_utf8_lossy(&self.rom[0x000..0x00C]).to_string();
        logger::info(format!("Game Title: {}", self.game_title));
        ...
        logger::info(format!("ARM9 ROM Offset: {:#010X}", self.arm9_rom_offset));
        logger::info(format!(
        ...
        logger::info(format!("ARM9 Size: {} bytes", self.arm9_size));

        logger::info("=== End ROM ===");

        if self.parse_error {
            logger::error("Failed to parse ROM");
            return false;
        }
    ...
    fn parse_u32(&mut self, offset: usize) -> u32 {
        let value: [u8; 4] = self.rom[offset..offset + 4].try_into().unwrap_or_else(|e| {
            ...
            logger::error(format!(
                "Failed to parse u32 at offset {:#010X}: {}",
                offset, e
            ));
            [0; 4]
        });
