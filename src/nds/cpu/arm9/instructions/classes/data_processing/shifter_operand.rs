...
pub fn parse(arm9: &mut Arm9, inst: &Instruction) -> ShifterOperand {
    let is_immediate = inst.get_bit(25);
    let mut carry_out = arm9.cpsr.get_carry();

    let second_source_operand = if is_immediate {
        let immed_8 = inst.get_bits(0, 7) as u32;
        // NOTE: rotate_imm must be even to be "legitimate"
        let rotate_imm = inst.get_bits(8, 11) as u32;
